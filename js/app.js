...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${image}></img>
      ...
      <h5>Total people rating: ${product.rating.count}</h5>
...
const displayDetails =(data) => {
  ...
  nameProduct.innerText =`Product Title:- ${data.title.slice(0,30)}`
  const delProduct = document.getElementById('product-history');
  delProduct.innerText =`Product Title:- ${data.description.slice(0,100)}`
